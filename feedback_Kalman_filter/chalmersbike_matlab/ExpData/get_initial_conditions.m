...
%         phidot_initCond = [phidot_initCond rad2deg(Data_exp1.PhiDot(1))];
        ...
        delta_initCond = [delta_initCond rad2deg(Data_exp1.Delta(max(1,find(Data_exp1.ControlInput,1)-1)))];
        phi_initCond = [phi_initCond rad2deg(Data_exp1.Phi(max(1,find(Data_exp1.ControlInput,1)-1)))];
        phidot_initCond = [phidot_initCond rad2deg(Data_exp1.PhiDot(max(1,find(Data_exp1.ControlInput,1)-1)))];
    end
end
...
figure;hist(phi_initCond,25);xlabel('Roll angle [deg]');set(findall(gcf,'-property','FontSize'),'FontSize',24)
figure;hist(delta_initCond,25);xlabel('Steering angle [deg]');set(findall(gcf,'-property','FontSize'),'FontSize',24)
figure;hist(phidot_initCond,25);xlabel('Roll rate [deg/s]');set(findall(gcf,'-property','FontSize'),'FontSize',24)
