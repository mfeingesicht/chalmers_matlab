...
if Estimations_errors==1
    ...
    xlabel('Time [s]')
    ylabel('Error [m]')
    error_direct_pos = ((X_BIKE.data-pos_meas.data).^2+(Y_BIKE.data-pos_meas.data).^2).^0.5;
    error_vel_int = ((X_BIKE.data-pos_vel_int.data).^2+(Y_BIKE.data-pos_vel_int.data).^2).^0.5;
...
%     error_direct_pos = ((X_BIKE.data-pos_meas.data(2:end,1)).^2+(Y_BIKE.data-pos_meas.data(2:end,2)).^2).^0.5;
%     error_vel_int = ((X_BIKE.data-pos_vel_int.data(2:end,1)).^2+(Y_BIKE.data-pos_vel_int.data(2:end,2)).^2).^0.5;
%     error_kalman = ((X_BIKE.data - X_BIKE_estim.data(2:end)).^2+(Y_BIKE.data - Y_BIKE_estim.data(2:end)).^2).^0.5;
%     plot(X_BIKE_estim.time(2:end), error_direct_pos, 'g')
%     plot(X_BIKE_estim.time(2:end), error_vel_int, 'b')
%     plot(X_BIKE_estim.time(2:end), error_kalman, 'k')
    grid on
