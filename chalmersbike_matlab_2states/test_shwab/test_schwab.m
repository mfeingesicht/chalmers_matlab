...
h = 0.8522; % height of center of mass [m]
b = 1.02;            % length between wheel centers [m]
c = 0.08;             % length between front wheel contact point and the extention of the fork axis [m]
lambda = deg2rad(72); % angle of the fork axis [deg]
a = 0.3496;           % distance from rear wheel to frame's center of mass [m]
...
%% Schwab
num_schwab = [0.0287 4*0.4190 0.9478*4*4-0.3155];
den_schwab = [1 0 -9.826];
systf_schwab = tf(num_schwab,den_schwab);
[A_schwab,B_schwab,C_schwab,D_schwab] = tf2ss(num_schwab,den_schwab),