...
lqr_gains = zeros(length(velocity_lqr),3);
...
if fork_dynamics_lqr==1
    ...
    for i = 1:length(velocity_lqr)
        ...
        B_lqr = [0; 1; (a_real*v*sin(lambda_real))/(h_real*b_real)];
        sys_con = ss(A_lqr,B_lqr,C,D);
        sys_dis = c2d(sys_con,Ts);

        K_lqr = lqrd(A_lqr,B_lqr,Q,R,Ts);
        lqr_gains(i,:) = K_lqr;

        sysc = ss(A_lqr,B_lqr,C,D);
        sysd = c2d(sysc,Ts);
    end
    ...
    for i = 1:length(velocity_lqr)
        ...
        B_lqr = [0; 1; a_real*v/(h_real*b_real)];
        sys_con = ss(A_lqr,B_lqr,C,D);
        sys_dis = c2d(sys_con,Ts);
...
if plots(1)==1
    ...
    for i = 1:length(velocity_eig)
        ...
        K_lqr = lqrd(A_lqr,B_lqr,Q,R,Ts);     
        lqr_gains_eig(i,:) = K_lqr;

        closed_loop_eigenvalues(:,i) = eig(sys_dis.A-sys_dis.B*K_lqr);
    end
